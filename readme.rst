...
How to Run the Code and Obtain Airway Measurements
========

1. Obtaining Detailed Airway Measurements

Execute the measurement_final.m file to generate the CSV file that contains measurements for each airway tube.  To do this, follow these steps:

- Adjust the path to AirQuant package (located on line 2 in the measurement_final.m file).

- Ensure measurement_final.m takes in 3 NifTi input files : CT-scan (referenced on line 8), segmented airway (referenced on line 9), and skeleton (referenced on line 10).

- To create the skeleton file required for this process, execute the ct2skel.py file, using the CT-scan NIfTi file (as mentioned on line 9). This will produce the skeleton NIfTi file indicated on line 10.

- The results from running measurement_final.m will be saved in CSV format, as specified on line 22.

2. Generating a Summary of Airway Measurement

Run the summarycsv.py file to create a CSV file containing summary statistics of measurement CSV file generated by measurement_final.m.
